...
void connect_mqttServer() {
  while (!client.connected()) {
    ...
    if (client.connect("node3_sensor")) {
...
void loop() {
  ...
  if (millis() - last_time > 5000) {
    ...
    client.publish("raspi/sensors", String("{\"room\": 1, \"node_id\": 3, \"current_light\": " + String(new_data) + ", \"light_mean\": " + String(mean) + ", \"recent_changes\": " + String(sigma) + "}").c_str());
  }
}
